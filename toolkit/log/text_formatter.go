...
func (f *TextFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if entry.HasCaller() {
		fixedKeys = append(fixedKeys, logrus.FieldKeyFunc, logrus.FieldKeyFile)
	}
...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, data Fields, timestampFormat string) {
	var levelColor int
	switch entry.Level {
...
func (f *TextFormatter) appendValue(b *bytes.Buffer, value any) {
	...
	if !ok {
		if j, ok := value.(json.RawMessage); ok {
			stringVal = string(j)
		} else {
